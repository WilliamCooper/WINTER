### plot 6: ambient pressures
RPlot6 <- function (data) { 
  ## needs PSFD, PSFRD, PSFDC, PSFC, PS_A
  op <- par (mar=c(2,4,1,4)+0.1,oma=c(1.1,0,0,0))
  layout(matrix(1:2, ncol = 1), widths = 1, heights = c(5,6))
  plotWAC (DF <- data[, c("Time", "PSFD", "PSFRD")], 
           col=c('blue', 'skyblue', 'red', 'red'), ylab='pressure  PSy [hPa]',legend.position=NA)
  # pulling legend out of plotWAC to increase font size
  legend('bottomright',c("PSFD", "PSFRD"),col=c("blue","skyblue"),text.col=c("blue","skyblue"),lty=1,lwd=c(2,1),cex=0.75)
#  points (data$Time, (data$PSFD-data$PSFRD)*50+600, type='l', col='red')
#  axis (4, at=c(500,600,700), labels=c("-2", "0", "2"), col='red', col.axis='red')
  par(new=T)
  plot (data$Time, (data$PSFD-data$PSFRD), type='l', col='red',axes=F,xlab='',ylab='',ylim=c(-4,4))
  axis (4, col='red', col.axis='red')
  mtext("PSFD - PSFRD [hPa]",4,col='red',3)
#  hline (500, 'red'); hline (700, 'red')
  abline (h=c(-2,2),col='red',lty=2)
  legend ("bottomleft", 
#          legend=c("(PSFD-PSFRD)*50+600", "+/-2 hPa"),
          legend=c("(PSFD-PSFRD)", "+/-2 hPa"),
          lty=c(1,2), cex=0.75,
          col=c('red', 'red'))
  op <- par (mar=c(5,4,1,4)+0.1)
  plotWAC (data[, c("Time", "PSFDC", "PSFC", "PS_A")], 
           col=c('blue', 'skyblue', 'darkgreen'), ylab="corrected PSyC [hPa]",legend.position=NA)
  # pulling legend out of plotWAC to increase font size
  legend('bottomright',c("PSFDC", "PSFC","PS_A"),col=c("blue","skyblue","darkgreen"),text.col=c("blue","skyblue","darkgreen"),lty=1,lwd=c(2,1,1),cex=0.75)
#  points (data$Time, (data$PSFC-data$PSFDC)*50+700, type='l', col='red')
#  axis (4, at=c(650,700,750), labels=c("-1", "0", "1"), col='red', col.axis='red')
  par(new=T)
  plot (data$Time, (data$PSFC-data$PSFDC), type='l', col='red',axes=F,xlab='',ylab='',ylim=c(-2,2))
  axis (4, col='red', col.axis='red')
  mtext("PSFC - PSFDC [hPa]",4,col='red',3)
#  hline (650, 'red'); hline (750, 'red')
  abline (h=c(-1,1),col='red',lty=2)
  legend ("bottomleft", 
#          legend=c("(PSFC-PSFDC)*50+700", "+/-1 hPa"),
          legend=c("(PSFC-PSFDC)", "+/-1 hPa"),
          lty=c(1,2), cex=0.75,
          col=c('red', 'red'))
  title (sprintf ("mean difference PSFC-PSFDC: %.1f;  PS_A-PSFC: %.1f", 
                  mean (data$PSFC-data$PSFDC, na.rm=TRUE),
                  mean (data$PS_A-data$PSFDC, na.rm=TRUE)), cex.main=0.75)
  mtext(paste(FigFooter,'generated by RPlot6',FigDatestr),1,outer=T,cex=0.75)
}


CircleSearch <- function (data) {
      ...
      if (s[j+1]-s[j] > 3) {
        ...
        if (Turn > 330) {
          print (sprintf( "possible circle:         %s--%s, %.0f-deg turn", 
                          ...
                          Turn))
          if (returnStart < 0) {returnStart <- startTime}
          returnEnd <- endTime
        }
  ...
  if (returnStart >= 0) {
    returnStart <- strftime(returnStart, format="%H%M%S", tz='UTC')
    returnEnd <- strftime(returnEnd, format="%H%M%S", tz='UTC')
  }
  return (c(returnStart, returnEnd))
}
